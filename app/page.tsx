...
export default function Home() {
  ...
  const inputClass =
    'w-full rounded-2xl border border-white/30 bg-white/10 p-4 text-white placeholder-white/60 outline-none backdrop-blur focus:border-white focus:ring-2 focus:ring-white/40';
  const btnClass =
    'rounded-full border border-white/30 bg-white/10 px-4 py-2 text-white shadow-sm backdrop-blur hover:bg-white/15 disabled:opacity-60';
  ...
  async function submit(kind: 'message' | 'review') {
    ...
    try {
      const res = await fetch('/api/submit', {
        ...
        // ✅ send `content` (the API expects this key)
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="leading-relaxed">
              Thank you—your {thanksKind === 'message' ? 'message' : 'letter'} was received and is now
              pending moderation.{' '}
              <a
      ...
      <div className="mx-auto max-w-5xl px-4 py-8">
        <header className="mb-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <nav className="mt-1 flex flex-wrap items-center gap-3 text-sm text-white/80">
              <a className={linkClass} href="/wall/messages">
                Unspoken words
              </a>
              <span className="opacity-80">•</span>
              <a className={linkClass} href="/wall/reviews">
                Letters to Geloy
              </a>
            </nav>
          </div>
        ...
        <section className="space-y-3">
          ...
          <div className="flex items-center justify-between text-sm text-white/70">
            ...
            <button onClick={() => submit('message')} disabled={loading} className={btnClass}>
              Submit anonymously
            </button>
          </div>
        </section>
        ...
        <section className="mt-8 space-y-3">
          ...
          <div className="flex items-center justify-between text-sm text-white/70">
            ...
            <button onClick={() => submit('review')} disabled={loading} className={btnClass}>
              Send letter
            </button>
          </div>
        </section>
